...
namespace AgrotutorAPI.Test
{
    ...
    public class AgrotutorTest
    {
      //  DbContextOptions<AgrotutorContext> _dbInMemoryOption;
        ...
        {
            _mediaItemList = new List<MediaItem>()
            {
                new MediaItem()
                {
                    Id = Guid.NewGuid(),
        ...
        {
            Mock<IHostingEnvironment> mockValidator = new Mock<IHostingEnvironment>();
        ...
        {
            var dbInMemoryOption = new DbContextOptionsBuilder<AgrotutorContext>().UseInMemoryDatabase(databaseName: "RetrieveList").Options;
            using (var context = new AgrotutorContext(dbInMemoryOption))
            {
                if (!context.Plots.Any())
                {
                    context.Plots.AddRange(
                        new Plot
                        {
                            Id = 1,
                            DeviceID="deviceIDTest",
                        ...
                        new Plot
                        {
                            ...
                            Activities = new List<Activity>
                            {
                                new Activity
                                {
                                    Id = 2,
                                    Name = "ActivityTest2",
                                    ActivityType = ActivityTypeDto.Harvest,
                                    AmountApplied = "AmountAppliedTest2",
                                    AmountSold = "AmountSoldTest2",
                                    ...
                                    NumberOfSeeds = 10,
                                    Price = 300,
                                    ProductObtained = "ProductObtainedTest2",
                                    Sown = "SownTest2",
                                    SellingPrice = 100,
                                    WeightOfSeeds = 400,
                                    Yield = "Yield2",
        ...
        public void UploadPictures()
        {
            ...
            var result = _pictureRepository.UploadImages(_mediaItemList);
        ...
        {
            var dbInMemoryOption = new DbContextOptionsBuilder<AgrotutorContext>().UseInMemoryDatabase(databaseName: "CreatPlotWithoutImages").Options;
            using (var context = new AgrotutorContext(dbInMemoryOption))
            {
                var rep = new PlotRepository(context);
                rep.AddPlot(_newPlot);
        ...
        {
            IntializaPictureRepository();
            var dbInMemoryOption = new DbContextOptionsBuilder<AgrotutorContext>().UseInMemoryDatabase(databaseName: "CreatPlotWithImages").Options;
            using (var context = new AgrotutorContext(dbInMemoryOption))
            {
                var rep = new PlotRepository(context);
                var uploadResult = _pictureRepository.UploadImages(_newPlotWithImages.MediaItems);
                if (uploadResult)
                {
                 var res=   JsonConvert.SerializeObject(_newPlotWithImages);
                    rep.AddPlot(_newPlotWithImages);
                    var saveResult = context.SaveChanges();
                    var result = rep.GetPlots();
                    var resultCount = result.Count();
                    Assert.IsTrue(saveResult > 0);
